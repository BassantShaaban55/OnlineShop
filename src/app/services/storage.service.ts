...
export class StorageService {
  ...
  getLikesQuantityFromLocalStorage(): number {
    return JSON.parse(localStorage.getItem('likes') || '[]');
  }

  saveLikeInLocalStorage(){
   let previousLikesNumber=this.getLikesQuantityFromLocalStorage();
   let newLikesNumber=Number(previousLikesNumber)+1;
   localStorage.setItem("likes",JSON.stringify(newLikesNumber));
  }
